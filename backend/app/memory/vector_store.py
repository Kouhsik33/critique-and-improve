...
class MemoryStore:
    ...
    def _initialize(self, settings: VectorStoreSettings):
        ...
        if openai_key:
            ...
            print("MemoryStore disabled: no embedding API key found (OPENAI_API_KEY or GEMINI/GOOGLE_API_KEY)")
            self._embedder = None
        ...
        if os.path.exists(index_path):
            try:
                ...
                # Validate embedding dimensionality to avoid FAISS assertion errors
                try:
                    test_vec = self._embedder.embed_query("dimension_check")
                    expected_dim = len(test_vec)
                    actual_dim = getattr(self._vector_store.index, "d", None)
                    if actual_dim is not None and actual_dim != expected_dim:
                        print(
                            f"Embedding dimension mismatch (index={actual_dim}, embedder={expected_dim}). "
                    ...
                    print(f"Failed to validate embedding dimensions: {e}. Resetting vector store.")
                    self._vector_store = None
                    self._memory_index = {}
            except Exception as e:
