...
class DatabaseManager:
    ...
    def get_metrics(cls, workflow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # If no system metrics yet, derive a lightweight snapshot
            if not system_metrics:
                total_token_usage = sum(m.token_count or 0 for m in agent_metrics)
                iteration_count = 0
                if agent_metrics:
                    iteration_count = max((m.iteration or 0) for m in agent_metrics)
                if idea_metrics:
                    iteration_count = max(iteration_count, max((m.iteration or 0) for m in idea_metrics))
                system_metrics = [
                    SystemMetric(
                        id="derived",
                        workflow_id=workflow_id,
        ...
        "idea_metrics": [
            {
                ...
                "token_efficiency": (m.total_token_usage or 0) / max(1, m.iteration_count or 1),
            }
